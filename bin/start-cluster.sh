...
DOCKER_RIAK_BASE_HTTP_PORT=${DOCKER_RIAK_BASE_HTTP_PORT:-11800}

if docker ps -a | grep "luman75/riak" >/dev/null; then
  echo ""
...
do
  ...
  if [ "${index}" -gt "1" ] ; then
    docker run -e "DOCKER_RIAK_CLUSTER_SIZE=${DOCKER_RIAK_CLUSTER_SIZE}" \
               ...
               --link "riak01:seed" \
               --name "riak${index}" \
               -d luman75/riak:2.0.4 > /dev/null 2>&1
  else
    docker run -e "DOCKER_RIAK_CLUSTER_SIZE=${DOCKER_RIAK_CLUSTER_SIZE}" \
               -e "DOCKER_RIAK_AUTOMATIC_CLUSTERING=${DOCKER_RIAK_AUTOMATIC_CLUSTERING}" \
               -e "DOCKER_RIAK_BACKEND=${DOCKER_RIAK_BACKEND}" \
...
echo
echo "Start Registering nodes in cluster"
echo
...
do
  ...
  if [ "${index}" -gt "1" ] ; then
    echo "  Registering node riak${index} in cluster"
    docker exec -it "riak${index}" "/sbin/automatic-clustering.sh"
  else
    docker exec -it "riak${index}" "/sbin/register-types.sh"
  fi
done

echo "Node registered successfuly"
