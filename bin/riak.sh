...
echo "riak.sh DOCKER_RIAK_CLUSTER_SIZE=${DOCKER_RIAK_CLUSTER_SIZE}  SEED_PORT_8098_TCP_ADDR=${SEED_PORT_8098_TCP_ADDR}" >> /tmp/automatic

# Start Riak
exec /sbin/setuser riak "$(ls -d /usr/lib/riak/erts*)/bin/run_erl" "/tmp/riak" "/var/log/riak" "exec /usr/lib/riak/bin/riak console"
